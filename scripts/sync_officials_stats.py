#!/usr/bin/env python3
"""同步三个智能体统计数据 → data/officials_stats.json"""
...
# 三个智能体定义
OFFICIALS = [
    {'id':'gege',   'label':'鸽鸽',   'role':'日常秘书', 'emoji':'🕊️','rank':'一级'},
    {'id':'goutou', 'label':'狗头',   'role':'项目经理',   'emoji':'🐕','rank':'一级'},
    {'id':'heinu',  'label':'黑奴',   'role':'资深开发',   'emoji':'👨‍💻','rank':'一级'},
]
...
def calc_cost(ss, model):
    p = MODEL_PRICING.get(model, MODEL_PRICING['anthropic/claude-sonnet-4-6'])
    usd = (ss['tokens_in']/1e6*p['in'] + ss['tokens_out']/1e6*p['out']) \
         + ss['cache_read']/1e6*p['cr'] + ss['cache_write']/1e6*p['cw']
    return round(usd, 4)

def get_task_stats(org_label, tasks):
    done   = [t for t in tasks if t.get('state')=='Done' and t.get('org')==org_label]
    active = [t for t in tasks if t.get('state') in ('Executing','Review','Assigned') and t.get('org')==org_label]
